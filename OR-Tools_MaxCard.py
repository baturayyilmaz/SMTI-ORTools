...
def main():
    ...
    # inputFileName = r"TestInputs/input8.txt"
    inputFileName = ""
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        print("Cardinality:", solver.ObjectiveValue(), "\n")
        print('Solution:')
        for i in range(numberOfMan):
            for j in range(numberOfWoman):
                if solver.BooleanValue(matching[i][j]):
                    print("m" + str(i + 1) + "-w" + str(j + 1))
                    # Matching.append([i+1, j+1]) # for checker
    else:
        print("No solution found.")

    # checker = Checker(ManList, WomanList, numberOfMan, numberOfWoman, Matching)
    # print(checker.CheckMatching())
    # print("Execution Time:", time.time() - START_TIME)
